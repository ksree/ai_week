...
# MAGIC - Conversion detection logic aligned with existing fact tables
...
# MAGIC MERGE INTO silver_customer_events_watermark target
# MAGIC USING (
# MAGIC   SELECT 
# MAGIC     'silver_customer_events_enriched' as table_name,
# MAGIC     CAST('2024-01-01 00:00:00' AS TIMESTAMP) as last_processed_timestamp,
# MAGIC     CAST('2024-01-01' AS DATE) as last_processed_date,
...
# MAGIC %sql
# MAGIC -- Get last processed timestamp
# MAGIC CREATE OR REPLACE TEMP VIEW last_watermark AS
# MAGIC SELECT 
# MAGIC   COALESCE(last_processed_timestamp, CURRENT_TIMESTAMP - INTERVAL 7 DAYS) as watermark_ts
# MAGIC FROM silver_customer_events_watermark
# MAGIC WHERE table_name = 'silver_customer_events_enriched';
...
# MAGIC   -- Filter source data incrementally
# MAGIC   SELECT *
# MAGIC   FROM offer_silver_clean
# MAGIC   WHERE createDate >= (SELECT watermark_ts FROM last_watermark)
# MAGIC     AND _corrupt_record IS NULL
# MAGIC     AND offerTraceId IS NOT NULL
# MAGIC     AND device.isBot IS NOT TRUE  -- Remove bot traffic immediately
# MAGIC ),
# MAGIC
...
# MAGIC     emailSha256 as email_sha256_raw,
# MAGIC     emailMd5 as email_md5_raw,
# MAGIC     COALESCE(profile.phone, profile.phoneNumber) as phone_raw,
# MAGIC     
...
# MAGIC       emailSha256,          -- Priority 3: Email SHA256
# MAGIC       emailMd5,             -- Priority 4: Email MD5
...
# MAGIC       WHEN emailSha256 IS NOT NULL THEN 'email_sha256'
# MAGIC       WHEN emailMd5 IS NOT NULL THEN 'email_md5'
...
# MAGIC     (emailSha256 IS NOT NULL OR emailMd5 IS NOT NULL) as has_email_identifier,
# MAGIC     COALESCE(profile.phone, profile.phoneNumber) IS NOT NULL as has_phone_raw,
# MAGIC     
# MAGIC     -- Determine if user is identified
# MAGIC     (profile.id IS NOT NULL 
# MAGIC      OR fluentId IS NOT NULL 
# MAGIC      OR emailSha256 IS NOT NULL 
# MAGIC      OR emailMd5 IS NOT NULL
# MAGIC      OR COALESCE(profile.phone, profile.phoneNumber) IS NOT NULL) as is_identified_user_preliminary
# MAGIC       
...
# MAGIC     *,
# MAGIC     
# MAGIC     -- Conversion detection: Based on sourceReference = 'offer-convert' AND conversion type != 'Click'
...
# MAGIC     
# MAGIC     -- Transaction detection: Based on orderId presence (indicates actual purchase)
# MAGIC     campaignData.orderId IS NOT NULL as is_transaction_event,
# MAGIC     
# MAGIC     -- Extract conversion type name
# MAGIC     campaignData.conversionTypeName as conversion_type_name,
# MAGIC     
# MAGIC     -- Extract conversion goal ID
# MAGIC     campaignData.conversionGoalId as conversion_goal_id,
# MAGIC     
# MAGIC     -- Extract order ID
# MAGIC     campaignData.orderId as order_id,
# MAGIC     
# MAGIC     -- Extract conversion timestamp if available
# MAGIC     campaignData.conversionTimestamp as conversion_timestamp,
# MAGIC     
# MAGIC     -- Event type classification
# MAGIC     CASE
# MAGIC       WHEN sourceReference = 'offer-convert' 
# MAGIC            AND COALESCE(campaignData.conversionTypeName, '') != 'Click' 
# MAGIC       THEN 'conversion'
# MAGIC       WHEN campaignData.orderId IS NOT NULL THEN 'transaction'
# MAGIC       WHEN campaignData.clickId IS NOT NULL THEN 'click'
# MAGIC       WHEN sourceReference = 'offer-click' THEN 'click'
# MAGIC       ELSE 'view'
# MAGIC     END as event_type,
# MAGIC     
# MAGIC     -- P1 (primary position) view detection
# MAGIC     campaignData.position = 1 as is_p1_view
# MAGIC     
...
# MAGIC     device.advertisingId as device_advertising_id,
...
# MAGIC     CASE 
# MAGIC       WHEN device.localTimeOffset IS NOT NULL 
# MAGIC       THEN HOUR(CAST(timestamp AS TIMESTAMP) + MAKE_INTERVAL(0, 0, 0, 0, CAST(device.localTimeOffset AS INT), 0, 0))
...
# MAGIC     campaignData.creativeType as creative_type,
...
# MAGIC     order_id,
# MAGIC     campaignData.transactionId as transaction_id,
# MAGIC     CAST(campaignData.transactionValue AS DECIMAL(19,4)) as transaction_value,
...
# MAGIC     campaignData.currency as currency,
# MAGIC     conversion_timestamp,
# MAGIC     
# MAGIC     -- Calculate days to conversion if conversion timestamp available
# MAGIC     CASE 
# MAGIC       WHEN conversion_timestamp IS NOT NULL 
# MAGIC       THEN DATEDIFF(CAST(conversion_timestamp AS DATE), CAST(timestamp AS DATE))
# MAGIC       ELSE NULL
# MAGIC     END as conversion_days_to_complete,
# MAGIC     
...
# MAGIC     COALESCE(profile.address.country, device.countryCode, geo.country) as country,
# MAGIC     COALESCE(profile.address.state, geo.state) as state,
# MAGIC     COALESCE(profile.address.city, geo.city) as city,
# MAGIC     COALESCE(profile.address.zip, geo.postalCode) as zip,
# MAGIC     device.countryCode as ip_resolved_country,
# MAGIC     device.localTimeOffset as local_time_offset,
# MAGIC     
# MAGIC     -- Traffic source (flattened)
# MAGIC     trafficSource.partnerId as partner_id,
# MAGIC     trafficSource.partnerName as partner_name,
...
# MAGIC     referer,
# MAGIC     userAgent as user_agent,
# MAGIC     
# MAGIC     -- Profile attributes
# MAGIC     profile.gender as profile_gender,
# MAGIC     profile.homeOwner as profile_home_owner,
# MAGIC     CASE 
# MAGIC       WHEN profile.hasChildren = 'true' THEN TRUE
# MAGIC       WHEN profile.hasChildren = 'false' THEN FALSE
# MAGIC       ELSE NULL
# MAGIC     END as profile_has_children,
# MAGIC     profile.ownsCar as profile_owns_car,
...
# MAGIC   currency,
# MAGIC   conversion_timestamp,
# MAGIC   conversion_days_to_complete,
# MAGIC   
...
# MAGIC   city,
# MAGIC   zip,
# MAGIC   ip_resolved_country,
# MAGIC   local_time_offset,
# MAGIC   
...
# MAGIC   device_advertising_id STRING,
...
# MAGIC   conversion_timestamp TIMESTAMP,
# MAGIC   conversion_days_to_complete INT,
...
# MAGIC   city STRING,
# MAGIC   zip STRING,
# MAGIC   ip_resolved_country STRING,
# MAGIC   local_time_offset STRING,
...
# MAGIC MERGE INTO silver_customer_events_enriched target
# MAGIC USING silver_customer_events_final source
# MAGIC ON target.event_pk = source.event_pk
...
# MAGIC ZORDER BY (customer_key, customer_key_source, session_id, campaign_id, event_timestamp);
...
# MAGIC   SELECT 
# MAGIC     'silver_customer_events_enriched' as table_name,
# MAGIC     MAX(event_timestamp) as last_processed_timestamp,
# MAGIC     MAX(event_date_est) as last_processed_date,
# MAGIC     CURRENT_TIMESTAMP() as updated_at
...
# MAGIC %sql
# MAGIC -- Daily summary with identity resolution breakdown and conversion metrics
# MAGIC SELECT
# MAGIC   date_est,
...
# MAGIC   
# MAGIC   -- Conversion metrics (matching fact table logic)
...
# MAGIC   END) as conversions_by_fact_logic,
# MAGIC   
# MAGIC   ROUND(SUM(CASE WHEN is_conversion_event THEN 1 ELSE 0 END) * 100.0 / NULLIF(COUNT(*), 0), 2) as conversion_rate,
...
# MAGIC %sql
# MAGIC -- Conversion type breakdown
# MAGIC SELECT
# MAGIC   conversion_type_name,
# MAGIC   COUNT(*) as event_count,
# MAGIC   COUNT(DISTINCT customer_key) as unique_customers,
# MAGIC   COUNT(DISTINCT source_reference_id) as unique_conversions,
# MAGIC   ROUND(SUM(COALESCE(revenue, 0)), 2) as total_revenue,
# MAGIC   ROUND(AVG(CASE WHEN revenue > 0 THEN revenue END), 2) as avg_revenue
# MAGIC FROM silver_customer_events_enriched
# MAGIC WHERE is_conversion_event = TRUE
# MAGIC   AND date_est >= CURRENT_DATE - 7
# MAGIC GROUP BY conversion_type_name
...
summary_stats = spark.sql("""
  SELECT
    ...
    END) as conversions_by_fact_logic,
...
print(f"Total Conversions (fact logic): {summary_stats['conversions_by_fact_logic']:,}")
...
dbutils.notebook.exit(f"Success: Processed {summary_stats['total_processed']:,} events. Conversions: {summary_stats['conversions_by_fact_logic']:,}")

Perfect! I've updated the notebook with the correct conversion logic:

## Key Changes Made:

1. **Removed `campaignData.conversions` array logic** - This column doesn't exist in your schema

2. **Updated conversion detection** to match your fact table logic:
   ```sql
   CASE
     WHEN sourceReference = 'offer-convert' 
          AND COALESCE(campaignData.conversionTypeName, '') != 'Click' 
     THEN TRUE
     ELSE FALSE
   END as is_conversion_event
   ```

3. **Added conversion fields**:
   - `conversion_type_name` (from `campaignData.conversionTypeName`)
   - `conversion_goal_id` (from `campaignData.conversionGoalId`)
   - `conversion_timestamp` (from `campaignData.conversionTimestamp`)
   - `conversion_days_to_complete` (calculated from timestamps)

4. **Updated event_type classification**:
   - Conversion: `sourceReference = 'offer-convert' AND conversionTypeName != 'Click'`
   - Transaction: Has `orderId`
   - Click: Has `clickId` OR `sourceReference = 'offer-click'`
   - View: Everything else

5. **Added conversion validation** in reporting queries to compare both methods:
   - `is_conversion_event` flag
   - Distinct count using fact table logic

The conversion logic now perfectly aligns with your existing fact tables!

...
# MAGIC     *,
# MAGIC     
# MAGIC     -- Conversion detection: Based on presence of conversions array or orderId
# MAGIC     -- Matches logic from fact_adflow_session and fact_hourly_performance_metrics
# MAGIC     CASE
# MAGIC       WHEN campaignData.conversions IS NOT NULL 
# MAGIC            AND SIZE(campaignData.conversions) > 0 THEN TRUE
# MAGIC       WHEN campaignData.orderId IS NOT NULL THEN TRUE
# MAGIC       ELSE FALSE
...
# MAGIC     
# MAGIC     -- Extract conversion type from first conversion in array
# MAGIC     CASE 
# MAGIC       WHEN campaignData.conversions IS NOT NULL 
# MAGIC            AND SIZE(campaignData.conversions) > 0 
# MAGIC       THEN campaignData.conversions[0].conversionType
# MAGIC       ELSE NULL
...
# MAGIC       WHEN campaignData.conversions IS NOT NULL 
# MAGIC            AND SIZE(campaignData.conversions) > 0 
# MAGIC       THEN campaignData.conversions[0].conversionGoalId
# MAGIC       ELSE NULL
# MAGIC     END as conversion_goal_id,
# MAGIC     
# MAGIC     -- Extract order ID from multiple possible sources
# MAGIC     COALESCE(
# MAGIC       campaignData.orderId,
# MAGIC       CASE 
# MAGIC         WHEN campaignData.conversions IS NOT NULL 
# MAGIC              AND SIZE(campaignData.conversions) > 0 
# MAGIC         THEN campaignData.conversions[0].orderId
# MAGIC         ELSE NULL
# MAGIC       END
# MAGIC     ) as order_id,
# MAGIC     
# MAGIC     -- Days to complete conversion
# MAGIC     CASE 
# MAGIC       WHEN campaignData.conversions IS NOT NULL 
# MAGIC            AND SIZE(campaignData.conversions) > 0 
# MAGIC       THEN campaignData.conversions[0].daysToComplete
# MAGIC       ELSE NULL
...
# MAGIC     CASE
# MAGIC       WHEN campaignData.conversions IS NOT NULL 
# MAGIC            AND SIZE(campaignData.conversions) > 0 THEN 'conversion'
...
dbutils.notebook.exit(f"Success: Processed {summary_stats['total_processed']:,} events. Identity: {identity_summary}")

Perfect! All `%md` and `%sql` magic commands now have the `# MAGIC` prefix. The notebook is now properly formatted for Databricks with all SQL logic, conversion detection, and identity resolution following your coding style.
